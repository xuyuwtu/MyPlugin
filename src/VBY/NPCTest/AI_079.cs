    ...
    public static void AI_079(this NPC npc)
    {
        ...
        if (npc.ai[0] == 1f)
        {
            ...
            else if (num1221 < num1222 - 15f)
            {
                if (num1221 == 180f)
                {
                    ...
                    velocity = velocity.RotatedBy((0f - direction) * ((float)Math.PI * 2f) / 6f);
                    ///Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), npc.Center.X, npc.Center.Y, velocity.X, velocity.Y, 455, 75, 0f, Main.myPlayer, direction * ((float)Math.PI * 2f) / 540f, npc.whoAmI);
                    float minSpeed = 0.3f;
                    Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), npc.Center.X, npc.Center.Y, velocity.X, velocity.Y, 455, 75, 0f, Main.myPlayer, direction * ((float)Math.PI * 2f) / (540f * Math.Max((npc.life / (float)npc.lifeMax), minSpeed)), npc.whoAmI);

                    npc.NewProjectile(npc.Center, velocity.RotatedByDegress(-direction * 120), 455, 75, direction * ((float)Math.PI * 2f) / (540f * Math.Max((npc.life / (float)npc.lifeMax), minSpeed)), npc.whoAmI);
                    npc.NewProjectile(npc.Center, velocity.RotatedByDegress(-direction * 240), 455, 75, direction * ((float)Math.PI * 2f) / (540f * Math.Max((npc.life / (float)npc.lifeMax), minSpeed)), npc.whoAmI);

                    npc.ai[2] = (velocity.ToRotation() + (float)Math.PI * 3f) * direction;
                    npc.netUpdate = true;
                }
                npc.localAI[1] += 0.05f;
                if (npc.localAI[1] > 1f)
        ...
        if (num1248 == 2)
        {
            ...
            if (npc.ai[1] == 0f)
            {
                ...
                npc.NewProjectile(npc.Center, vector223.RotatedByDegress(90), 455, 1, (float)Math.PI / 300f, npc.whoAmI);
                npc.NewProjectile(npc.Center, vector223.RotatedByDegress(180), 455, 1, (float)Math.PI / 300f, npc.whoAmI);
                npc.NewProjectile(npc.Center, vector223.RotatedByDegress(270), 455, 1, (float)Math.PI / 300f, npc.whoAmI);
            }
            npc.ai[1]++;
            if (npc.ai[1] >= 600f)
            {
