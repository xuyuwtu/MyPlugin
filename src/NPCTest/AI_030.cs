...
public static partial class NPCAIs
{
    public static void AI_030(this NPC npc)
    {
                ...
                else if (npc.position.Y + npc.height < Main.player[npc.target].position.Y && num414 < 400f)
                {
                    ...
                    if (npc.ai[3] >= num417)
                    {
                        ...
                        if (Main.netMode != 1)
                        {
                            ...
                            Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), vector43.X, vector43.Y, num411, num412, projType, attackDamage_ForProjectiles3, 0f, Main.myPlayer);

                            var velocity = new Vector2(num411, num412);
                            if (Main.rand.Next(2) == 0)
                            {
                                npc.NewProjectile(vector43, velocity, ProjectileID.MartianTurretBolt, attackDamage_ForProjectiles3);
                            }
                            npc.NewProjectile(vector43, velocity.RotatedByDegress(5), projType, attackDamage_ForProjectiles3);
                            npc.NewProjectile(vector43, velocity.RotatedByDegress(-5), projType, attackDamage_ForProjectiles3);
                        }
                    }
                }
        ...
        if (npc.ai[1] == 0f)
        {
            ...
            Vector2 projPosition = new(npc.position.X + npc.width * 0.5f, npc.position.Y + npc.height * 0.5f);
            float num429 = Main.player[npc.target].position.X + Main.player[npc.target].width / 2 - projPosition.X;
            float num430 = Main.player[npc.target].position.Y + Main.player[npc.target].height / 2 - 300f - projPosition.Y;
            if (NPC.IsMechQueenUp)
            {
                num427 = 14f;
                num429 = vector41.X;
                num430 = vector41.Y;
                num429 -= projPosition.X;
                num430 -= projPosition.Y;
            }
            ...
            projPosition = new Vector2(npc.position.X + npc.width * 0.5f, npc.position.Y + npc.height * 0.5f);
            num429 = Main.player[npc.target].position.X + Main.player[npc.target].width / 2 - projPosition.X;
            num430 = Main.player[npc.target].position.Y + Main.player[npc.target].height / 2 - projPosition.Y;
            ...
            if (npc.localAI[1] > 180f)
            {
                ...
                num429 *= num431;
                num430 *= num431;
                projPosition.X += num429 * 15f;
                projPosition.Y += num430 * 15f;
                ...
                Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), projPosition.X, projPosition.Y, num429, num430, projType, projDamage, 0f, Main.myPlayer);

                var velocity = new Vector2(num429, num430); 
                if (Main.rand.Next(3) == 0)
                {
                    npc.NewProjectile(projPosition, velocity, ProjectileID.Shadowflames, projDamage);
                }
                npc.NewProjectile(projPosition, velocity.RotatedByDegress(5), projType, projDamage);
                npc.NewProjectile(projPosition, velocity.RotatedByDegress(-5), projType, projDamage);
            }
            return;
        }
        ...
        if (Main.netMode != 1)
        {
            ...
            if (npc.localAI[1] > 60f)
            {
                ...
                Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), vector46.X, vector46.Y, num439, num440, projType, attackDamage_ForProjectiles5, 0f, Main.myPlayer);

                var velocity = new Vector2(num439, num440);
                if (Main.rand.Next(3) == 0)
                {
                    npc.NewProjectile(vector46, velocity.RotatedByDegress(10), projType, attackDamage_ForProjectiles5);
                    npc.NewProjectile(vector46, velocity.RotatedByDegress(-10), projType, attackDamage_ForProjectiles5);
                }
                npc.NewProjectile(vector46, velocity.RotatedByDegress(5), projType, attackDamage_ForProjectiles5);
                npc.NewProjectile(vector46, velocity.RotatedByDegress(-5), projType, attackDamage_ForProjectiles5);
            }
        }
        npc.ai[2] += 1f;
