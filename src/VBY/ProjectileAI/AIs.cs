...
public static partial class AIs
{
    ...
    {
        const int skipAi = 99;
        _ProjectileAIs[skipAi] = static (self) =>
        {
        ...
        var aiStyleField = type.GetField(nameof(Projectile.aiStyle))!;
        // self.aiStyle == \d{1,3}
        ...
        for (int i = 0; i < instructions.Count; i++)
        {
            ...
            if (match)
            {
                ...
                if (ldci4Value == skipAi)
                {
                    i = instructions.BinarySearch(new Instruction(ILOpCode.Ret, null, 0x23722), offsetComparer);
                }
            }
        }
        ...
        for (int i = 0; i < ranges.Count; i++)
        {
            ...
            {
                _ProjectileAIs[aiStyle] = (Action<Projectile>)Delegate.CreateDelegate(typeof(Action<Projectile>), null, (MethodInfo)rangeInstructions[1].Operand!);
                continue;
            }
            ...
            _ProjectileAIs[aiStyle] = (Action<Projectile>)newAiMethod.CreateDelegate(typeof(Action<Projectile>));
        }
    }
}
