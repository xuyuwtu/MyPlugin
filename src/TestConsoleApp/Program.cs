...
internal class Program
{
    ...
    {
        var assemblyBuilder = new PersistedAssemblyBuilder(new AssemblyName("ProjectileAI"), typeof(int).Assembly);
        var moduleBuilder = assemblyBuilder.DefineDynamicModule("ProjectileAI");
        var typeBuilder = moduleBuilder.DefineType("AIs", TypeAttributes.Abstract | TypeAttributes.Sealed);
        NPCBuild(moduleBuilder, typeBuilder);
        var type = typeBuilder.CreateType();
        //assemblyBuilder.GenerateMetadata(out var ilStream, out var mappedFieldData, out var pdbBuilder);
        assemblyBuilder.Save("ProjectileAI.dll");
    }
    static void NPCBuild(ModuleBuilder moduleBuilder, TypeBuilder typeBuilder)
    {
        ...
        for (int i = 0; i < ranges.Count; i++)
        {
            ...
            var rangeLastIsRet = rangeLastOpCode == ILOpCode.Ret;
            var rangeLastIsBranchToRet = EmitUtils.GetOpCode(rangeLastOpCode).OperandType is OperandType.InlineBrTarget or OperandType.ShortInlineBrTarget && ((Instruction)rangeLastInstruction.Operand!).OpCode == ILOpCode.Ret;
            ...
            var newAiMethod = typeBuilder.DefineMethod($"AI_{aiStyle.ToString().PadLeft(3, '0')}", MethodAttributes.Public | MethodAttributes.Static, typeof(void), aiActionParamTypes);
            ...
            foreach (var instruction in rangeInstructions)
            {
                ...
                if (!hasLeaveOrCondBrToRet && (instruction is { OpCode: ILOpCode.Leave or ILOpCode.Leave_s, Operand: Instruction targetInstruction } && targetInstruction.Offset == instructions[^1].Offset || EmitUtils.GetOpCode(instruction.OpCode).FlowControl == FlowControl.Cond_Branch))
                {
        ...
        //{
        //    Terraria.Main.npc[i] = new();
        //}
        //Terraria.Lighting._activeEngine = Terraria.Lighting.LegacyEngine;
        ////npc.SetDefaults(105); // 0
        //npc.SetDefaults(384); // 72
        //aiActions[npc.aiStyle](npc);
    }
    static void ProjectileBuild(ModuleBuilder moduleBuilder, TypeBuilder typeBuilder)
    {
        var npcType = typeof(Projectile);
        var aiMethod = npcType.GetMethod(nameof(Projectile.AI))!;
        var aiMethodBody = aiMethod.GetMethodBody()!;
        var module = npcType.Module;
        var instructions = EmitUtils.GetInstructionsFromBytes(aiMethodBody.GetILAsByteArray()!);
        EmitUtils.InstructionOperandTransform(module, instructions);
        var aiStyleField = npcType.GetField(nameof(Projectile.aiStyle))!;
        var findOpcodeFuncs = new Func<Instruction, bool>[]
        {
            static x => x.OpCode == ILOpCode.Ldarg_0,
            x => x.OpCode == ILOpCode.Ldfld && (FieldInfo)x.Operand! == aiStyleField,
        ...
        ranges.Add(new(foundIndices[^1] + 4, instructions.Count));
        var aiActions = new Action<Projectile>[ranges.Count];
        var aiActionParamTypes = new Type[] { npcType };
        var exceptionHandlerInfos = new ExceptionHandlerInfo[aiMethodBody.ExceptionHandlingClauses.Count];
        var exceptionIndices = new Queue<int>(aiMethodBody.ExceptionHandlingClauses.Count * 4);
        for (int i = 0; i < aiMethodBody.ExceptionHandlingClauses.Count; i++)
        ...
        for (int i = 0; i < ranges.Count; i++)
        {
            ...
            if (rangeInstructions is [{ OpCode: ILOpCode.Ldarg_0 }, { OpCode: ILOpCode.Callvirt }, _])
            {
                aiActions[aiStyle] = (Action<Projectile>)Delegate.CreateDelegate(typeof(Action<Projectile>), null, (MethodInfo)rangeInstructions[1].Operand!);
                Console.WriteLine(((MethodInfo)rangeInstructions[1].Operand!).Name);
                continue;
            }
            var rangeLastInstruction = rangeInstructions[^1];
            ...
            //var newAiMethod = new DynamicMethod($"AI_{aiStyle.ToString().PadLeft(3, '0')}", typeof(void), aiActionParamTypes);
            var newAiMethod = typeBuilder.DefineMethod($"AI_{aiStyle.ToString().PadLeft(3, '0')}", MethodAttributes.Public | MethodAttributes.Static, typeof(void), aiActionParamTypes);
            newAiMethod.DefineParameter(0, ParameterAttributes.None, "self");
            var oplocInstructions = new List<Instruction>();
            foreach (var instruction in rangeInstructions)
            {
                if (instruction.OpCode is (>= ILOpCode.Ldloc_0 and <= ILOpCode.Ldloc_3) or ILOpCode.Ldloc_s or ILOpCode.Ldloca_s or ILOpCode.Ldloc or ILOpCode.Ldloca
            ...
            il.Emit(EmitUtils.GetOpCode(retInstruction.OpCode));
            addedIns.Add(retInstruction);
            //aiActions[aiStyle] = (Action<Projectile>)newAiMethod.CreateDelegate(typeof(Action<Projectile>));
            Console.WriteLine(newAiMethod.Name);
        }
        //Terraria.Program.SavePath = Directory.GetCurrentDirectory();
        //Terraria.Main.player[Terraria.Main.myPlayer] = new();
...
[DebuggerDisplay("0x{Offset.ToString(\"x\"),nq}, {OpCode} {Operand}")]
...
#pragma warning restore CS0618 // 类型或成员已过时
    ...
    public static OpCode GetOpCode(int code) => (ushort)code >= 0xFE00 ? LargeOpCodes[(ushort)code - 0xFE00]!.Value : ShortOpCodes[(int)code]!.Value;
    public static OpCode GetOpCode(ILOpCode code) => GetOpCode((int)code);
