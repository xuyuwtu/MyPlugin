...
partial class AIs
{
    public static void AI_051(this NPC npc)
    {
        ...
        {
            npc.localAI[0] = 1f;
            _ = NPC.NewNPC(npc.GetSpawnSourceForNPCFromNPCAI(), (int)npc.Center.X, (int)npc.Center.Y, 263, npc.whoAmI);
            _ = NPC.NewNPC(npc.GetSpawnSourceForNPCFromNPCAI(), (int)npc.Center.X, (int)npc.Center.Y, 263, npc.whoAmI);
            _ = NPC.NewNPC(npc.GetSpawnSourceForNPCFromNPCAI(), (int)npc.Center.X, (int)npc.Center.Y, 263, npc.whoAmI);
        }
        ...
        if (npc.life > npc.lifeMax / 2)
        {
            ...
            if (flag41)
            {
                ...
                num781 = npc.GetAttackDamage_ForProjectiles(num781, num781 * 0.9f);
                vector99.X += num778 * 3f;
                vector99.Y += num779 * 3f;
                int num783 = Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), vector99.X, vector99.Y, num778, num779, num782, num781, 0f, Main.myPlayer);
                if (num782 != 277)
                {
                    Main.projectile[num783].timeLeft = 300;
                }
            }
            return;
        }
        ...
        if (Main.netMode != 1)
        {
            if (npc.localAI[0] == 1f)
            {
                ...
                for (int num786 = 0; num786 < num785; num786++)
                {
                    _ = NPC.NewNPC(npc.GetSpawnSourceForNPCFromNPCAI(), (int)npc.Center.X, (int)npc.Center.Y, 264, npc.whoAmI);
                }
                if (Main.expertMode)
                {
                    for (int num788 = 0; num788 < 200; num788++)
                    {
                        ...
                        {
                            for (int num789 = 0; num789 < num785 / 2 - 1; num789++)
                            {
