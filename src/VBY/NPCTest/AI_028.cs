    ...
    public static void AI_028(this NPC npc)
    {
        ...
        else
        {
            ...
            if (flag27)
            {
                ...
                int projType = ProjectileID.EyeLaser;
                if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                {
                    if(Main.rand.Next(3) == 0)
                    {
                        projType = ProjectileID.FrostBeam;
                    }
                }
                else
                {
                    ...
                    {
                        projType = ProjectileID.Shadowflames;
                    }
                }

                Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), vector39.X, vector39.Y, num381, num382, projType, damage, 0f, Main.myPlayer);

                var velocity = new Vector2(num381, num382);
                npc.NewProjectile(velocity.RotatedByDegress(2), projType, damage);
                npc.NewProjectile(velocity.RotatedByDegress(-2), projType, damage);
            }
        }
    }
