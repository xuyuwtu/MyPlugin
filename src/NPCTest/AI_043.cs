            ...
            if (Collision.CanHit(vector84, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height) && flag35)
            {
                ...
                //{
                    ...
                    if (Main.rand.Next(4) == 0)
                    {
                        if (Main.hardMode)
                        {
                            npcType = Utils.SelectRandom(Main.rand, NPCID.MossHornet, NPCID.TinyMossHornet, NPCID.LittleMossHornet, NPCID.BigMossHornet, NPCID.GiantMossHornet);
                        }
                        else
                        {
                            npcType = Main.rand.Next(231, 236);
                        }
                    }
                    int num660 = NPC.NewNPC(npc.GetSpawnSourceForNPCFromNPCAI(), (int)vector84.X, (int)vector84.Y, npcType);
                    NPC newNpc = Main.npc[num660];
                    newNpc.velocity = Main.player[npc.target].Center - npc.Center;
                    newNpc.velocity.Normalize();
                    newNpc.velocity *= 5f;
                    newNpc.CanBeReplacedByOtherNPCs = true;
                    newNpc.localAI[0] = 60f;
        ...
        else if (npc.ai[0] == 3f)
        {
            ...
            if (npc.ai[1] % num668 == num668 - 1 && npc.position.Y + npc.height < Main.player[npc.target].position.Y && Collision.CanHit(vector86, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
            {
                ...
                if (Main.netMode != 1)
                {
                    ...
                    num672 *= num674;
                    num673 *= num674;
                    int num677 = Projectile.NewProjectile(npc.GetSpawnSource_ForProjectile(), vector86.X, vector86.Y, num672, num673, 719, 11, 0f, Main.myPlayer);
                    Main.projectile[num677].timeLeft = 300;

                    var velocity = new Vector2(num672, num673);
                    Main.projectile[npc.NewProjectile(vector86, velocity.RotatedByDegress(10), 719, 11)].timeLeft = 300;
                    Main.projectile[npc.NewProjectile(vector86, velocity.RotatedByDegress(-10), 719, 11)].timeLeft = 300;


                    //Projectile.NewProjectile(npc.GetSpawnSourceForProjectileNPC(), npc.Center, velocity, 686, npc.damage, 0f);
                }
            }
