...
partial class AIs
{
    public static void AI_015(this NPC npc)
    {
        ...
        int num253 = Dust.NewDust(npc.position, npc.width, npc.height, 4, npc.velocity.X, npc.velocity.Y, 255, new Color(0, 80, 255, 80), npc.scale * 1.2f);
        ...
        int num255 = (int)(npc.lifeMax * 0.05);
        if (!(npc.life + num255 < npc.ai[3]))
        {
            return;
        }
        npc.ai[3] = npc.life;
        ...
        for (int num257 = 0; num257 < num256; num257++)
        {
            ...
            int num259 = NPC.NewNPC(npc.GetSpawnSourceForProjectileNPC(), x, y2, num258);
            Main.npc[num259].SetDefaults(num258);
            Main.npc[num259].velocity.X = Main.rand.Next(-15, 16) * 0.1f;
            Main.npc[num259].velocity.Y = Main.rand.Next(-30, 1) * 0.1f;
